...
(function(){var lb=0;window.addEventListener("keydown",function(e){if(e.keyCode===27||e.keyCode===4||e.key==="GoBack"||e.key==="Escape"){var t=Date.now();if(t-lb>60&&t-lb<750){e.preventDefault();e.stopImmediatePropagation();if(typeof window._yttv==="object"&&window._yttv){for(var k in window._yttv){if(window._yttv[k]?.instance?.resolveCommand){window._yttv[k].instance.resolveCommand({signalAction:{signal:"EXIT_APP"}});break;}}}try{window.close();}catch(err){}lb=0;return!1;}lb=t;}},!0);})();
